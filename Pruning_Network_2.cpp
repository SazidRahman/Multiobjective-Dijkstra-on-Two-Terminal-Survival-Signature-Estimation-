...
// ---- PARAMETERS -------------------------------------------------------------
const int V = 30;              // Total number of nodes
const int v = V-2;              // Nodes that can fail (source and sink don't fail)
const int m1 = 7;              // Number of nodes in the first group/class of nodes
const int m2 = 7;               // Number of nodes in the second group/class of nodes
...
private:
    int _size{};
    vector<array<int, N>> vect = {{-1,0,0,0,0,0}};;
...
void node_labeling(vector<int>& group1, vector<int>& group2, vector<int>& group3, Graph g);

bool NewCandLabel (Graph g, int i, array<int, N> l_star, int r, array<int, 6> &l_new, array<int, 3> &ub, MaxHeap* Heap);

void RelaxProcess (Graph g, int i, array<int, N> l_star, array<int, 3> &ub, MaxHeap* Heap);

vector<array<int, 6>> lable;

void print_signature(Graph g, int m1, int m2, int m3);

vector<int> sample_WO (vector<int> m, int sizeComb, int j);

void BO_maxCapPath (vector<int> perm1, vector<int> perm2, vector<int> perm3, Graph g, MaxHeap* Heap);
...
// Either assign the nodes to each class "manually" or following the piece of code below
    ...
    std::vector<int> class_1 = {1, 17, 18, 19, 5, 11, 12};
    std::vector<int> class_2 = {8, 21, 10, 6, 7, 13, 14};
    std::vector<int> class_3 = {15, 16, 2, 3, 4, 20, 9, 22, 23, 24, 25, 26, 27, 28};
...
// Assigns the first m1 even nodes to the first class and the remaining m2 nodes to the second class
    ...
    for (int j=1; j <= M; j++)
    {
        ...
        BO_maxCapPath (perm1, perm2, perm3, g, Heap);
...
void MaxHeap::shiftUp(int i){
    if (i > _size) return;
    if (i == 1) return;
    if (vect.at(i)[1] > vect.at(p(i))[1] || (vect.at(i)[1] == vect.at(p(i))[1] && vect.at(i)[2] > vect.at(p(i))[2]) || (vect.at(i)[1] == vect.at(p(i))[1] && vect.at(i)[2] == vect.at(p(i))[2] && vect.at(i)[3] > vect.at(p(i))[3])){
...
void MaxHeap::shiftDown(int i) {
    ...
    if (l(i) <= _size && (vect.at(i)[1] < vect.at(l(i))[1] || (vect.at(i)[1] == vect.at(l(i))[1] && vect.at(i)[2] < vect.at(l(i))[2]) || (vect.at(i)[1] == vect.at(l(i))[1] && vect.at(i)[2] == vect.at(l(i))[2] && vect.at(i)[3] < vect.at(l(i))[3])) ) {
        swapId = l(i);
    }

    if (r(i) <= _size && (vect.at(swapId)[1] < vect.at(r(i))[1] || (vect.at(swapId)[1] == vect.at(r(i))[1] && vect.at(swapId)[2] < vect.at(r(i))[2]) || (vect.at(swapId)[1] == vect.at(r(i))[1] && vect.at(swapId)[2] == vect.at(r(i))[2] && vect.at(swapId)[3] < vect.at(r(i))[3])) ) {
        swapId = r(i);
    }
...
bool NewCandLabel (Graph g, int i, array<int, N> l_star, int r, array<int, N> & l_new, array<int, 3> &ub, MaxHeap* Heap)
{
    bool key = false;
    ...
    for (list<int>::iterator j = pred.begin(); j != pred.end(); j++)
    {
        ...
        for (list<array<int, N>>::iterator lj=ndpoints[*j].begin(); lj != ndpoints[*j].end(); lj++)
        {
                ...
                if (f[0] > d[0] || (f[0] == d[0] && f[1] > d[1]) || (f[0] == d[0] && f[1] == d[1] && f[2] > d[2])){
...
void node_labeling (vector<int>& perm1, vector<int>& perm2, vector<int>& perm3, Graph g)
{
...
void RelaxProcess (Graph g, int i, array<int, N> l_star, array<int, 3> &ub, MaxHeap* Heap)
{
    ...
    for(list<int>::iterator j = succ.begin(); j != succ.end(); j++)
    {
        ...
        if (f[0] > d[*j][0] || (f[0] == d[*j][0] && f[1] > d[*j][1]) || (f[0] == d[*j][0] && f[1] == d[*j][1] && f[2] > d[*j][2]))
        {
            //l = ndpoints[*j].back();
...
void BO_maxCapPath(vector<int> perm1, vector<int> perm2, vector<int> perm3, Graph g, MaxHeap* Heap)
{
    ...
    fill(&d[0][0], &d[0][0] + V*3, 0);
    fill(&Nindex[0], &Nindex[0] + V, 0);
    fill(&InH[0], &InH[0] + V, false);

    node_labeling (perm1, perm2, perm3, g);

    d[0][0] = V;
    d[0][1] = V;
    d[0][2] = V;

    array<int, N> l = {{0, d[0][0], d[0][1], d[0][2], 0, 0}};
...
void signature_estimation()
{
  ...
  for(int i=0; i<=m1; i++)
  {
    for(int j=0; j<=m2; j++)
    {
        for(int k=0; k<=m3; k++)
        {
            if (survSig_filing[i][j][k] == true)
                survSig[i][j][k] = survSig[i][j][k] + 1;
        }
    }
   }
}
...
{
    ofstream myfile;
	myfile.open("signature_Network_1.txt");
